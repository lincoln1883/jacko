...
describe('SessionsIndex', () => {
  ...
  describe('Rendering', () => {
    it('renders the sessions page with correct title and description', () => {
      ...
      expect(screen.getByText('Manage your active login sessions across different devices.')).toBeInTheDocument();
    });

    it('sets the correct page title', () => {
      renderWithInertia(<SessionsIndex sessions={mockSessions} />);
      expect(document.querySelector('title')).toHaveTextContent('Active Sessions');
    });
    ...
    it('renders security tip information', () => {
      ...
      expect(screen.getByText(/If you see any sessions you don't recognize/)).toBeInTheDocument();
    });
  });
  ...
  describe('Sessions list', () => {
    it('displays all sessions with correct information', () => {
      ...
      // Check timestamps
      expect(screen.getByText('Started: January 15, 2025 at 10:30 AM')).toBeInTheDocument();
      expect(screen.getByText('Started: January 14, 2025 at 2:15 PM')).toBeInTheDocument();
      expect(screen.getByText('Started: January 13, 2025 at 8:45 AM')).toBeInTheDocument();
    });
  ...
  describe('User agent formatting', () => {
    ...
    it('formats mobile user agent correctly', () => {
      const mobileSession: Session[] = [
        {
          id: '1',
          user_agent: 'Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) Mobile/15E148',
          ip_address: '192.168.1.100',
          created_at: 'January 15, 2025 at 10:30 AM',
        },
      ];
  ...
  describe('Session logout functionality', () => {
    it('shows confirmation dialog when logging out a session', async () => {
      ...
      const { user, mockUseForm } = renderWithInertia(<SessionsIndex sessions={mockSessions} />);
      ...
      expect(window.confirm).toHaveBeenCalledWith('Are you sure you want to log out this session?');
    });

    it('calls delete endpoint when user confirms logout', async () => {
      const mockDelete = vi.fn();
      const { user } = renderWithInertia(<SessionsIndex sessions={mockSessions} />, {
        formOptions: {
          delete: mockDelete,
          processing: false,
        }
  ...
  describe('Accessibility', () => {
    ...
    it('provides meaningful session information', () => {
      ...
      mockSessions.forEach(session => {
        expect(screen.getByText(`IP Address: ${session.ip_address}`)).toBeInTheDocument();
        expect(screen.getByText(`Started: ${session.created_at}`)).toBeInTheDocument();
      });
    });
  });
