...
RSpec.describe TradesPersonProfile, type: :model do
  let(:user) { create(:user, :tradesperson) }
  let(:profile) { build(:trades_person_profile, user: user) }
  ...
  describe "validations" do
    ...
    it "validates uniqueness of user" do
      create(:trades_person_profile, user: user)
      duplicate_profile = build(:trades_person_profile, user: user)
  ...
  describe "enums" do
    it "defines availability_status enum" do
      expect(TradesPersonProfile.availability_statuses).to eq({
        "available" => 0,
        "busy" => 1,
        "unavailable" => 2,
        "booked" => 3
  ...
  describe "scopes" do
    before do
      create(:trades_person_profile, :completed, availability_status: :available, years_experience: 5)
      create(:trades_person_profile, :incomplete, availability_status: :busy, years_experience: 10)
      create(:trades_person_profile, availability_status: :unavailable, active: false, years_experience: 2)
    end

    describe ".active" do
      it "returns only active profiles" do
        expect(TradesPersonProfile.active.count).to eq(2)
      end
    end

    describe ".available" do
      it "returns only available profiles" do
        expect(TradesPersonProfile.available.count).to eq(1)
      end
    end

    describe ".completed" do
      it "returns only completed profiles" do
        expect(TradesPersonProfile.completed.count).to eq(1)
      end
    end

    describe ".with_experience" do
      it "returns profiles with at least the specified experience" do
        expect(TradesPersonProfile.with_experience(8).count).to eq(1)
      end
    end
  end
  ...
  describe "display methods" do
    let(:profile) { create(:trades_person_profile, hourly_rate: 75.50, years_experience: 10) }
