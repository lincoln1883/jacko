...
RSpec.describe SupplierProfilesController, type: :controller do
  ...
  describe "GET #show" do
    context "when user is a supplier" do
      it "renders the show template with profile data" do
        ...
        expect_inertia_render("Profile/SupplierShow")
      end

      it "creates profile if it doesn't exist" do
        user.supplier_profile&.destroy
        expect(user.supplier_profile).to be_nil

        get :show

        expect(response).to have_http_status(:success)
        expect(user.reload.supplier_profile).to be_present
      end
    end
    ...
    context "when user is not a supplier" do
      ...
      it "redirects with access denied" do
        ...
        expect(flash[:alert]).to eq("Access denied. Only suppliers or contractors can access this page.")
      end
    end
  end
  ...
  context "when user is a supplier" do
    it "renders the edit template" do
      ...
      expect(response).to have_http_status(:success)
      expect_inertia_render("Profile/SupplierEdit")
    end
  end
  ...
  context "when client wants to switch roles" do
    ...
    it "handles role switch to supplier" do
      get :edit, params: {switch_role: "true"}

      expect(response).to redirect_to(edit_profile_supplier_path)
      expect(flash[:notice]).to include("switched to Supplier")
      expect(client_user.reload.role).to eq("supplier")
    end
  end
  ...
  describe "PATCH #update" do
    ...
    context "with valid parameters" do
      let!(:existing_profile) { create(:supplier_profile, user: user) }
      ...
      it "marks profile as completed when requirements are met" do
        ...
        profile = user.supplier_profile.reload
      ...
      it "updates skills when changing skill selection" do
        ...
        new_params[:supplier_profile][:skill_ids] = [skills.second.id, skills.third.id]
    ...
    context "when user is not authorized" do
      let(:other_user) { create(:user, :supplier) }
      let!(:other_profile) { create(:supplier_profile, user: other_user) }
      let!(:current_user_profile) { create(:supplier_profile, user: user) }
      ...
      it "redirects with authorization error" do
        ...
        allow(other_user).to receive(:supplier_profile).and_return(current_user_profile)

        patch :update, params: valid_params

        expect(response).to redirect_to(profile_supplier_path)
